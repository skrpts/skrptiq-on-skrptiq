...
id: idea-analysis
title: Idea Analysis
description: "Evaluates a plain-language skrpt idea: feasibility, required node types, catalogue overlap, complexity assessment"
...
5. **Catalogue overlap** — checks whether existing Hub skrpts already cover this use case (fully, partially, or not at all)
...
The analysis is based on the skrpt platform's current capabilities. It cannot assess ideas that depend on features not yet built (e.g. workflow chaining, persistent memory between runs). The catalogue overlap check depends on the user providing existing skrpt names — it cannot query the Hub directly.
